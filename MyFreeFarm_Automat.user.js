...
var neededVersionBerater = "2.10.65";
var neededVersionFunctionFile = "2.1.12";
var GM_Home = GM_info["script"]["namespace"];
var GM_Source = GM_info["script"]["namespace"];
// [version,date,[[de,en],[de,en],...]]
var CHANGELOG = [["2.0", "29.05.2014", [["Migration nach openuserjs.org", "Migration to openuserjs.org"], ["Script wird jetzt auf GitHub entwickelt", "Script now developed on GitHub"]]],
...
var LOGGING_ARBITER = false;
var PRODSTOP = -1;
var PRODSTOPold = 66; // TODO remove
var MILLSTOPold = 0; // TODO remove
var ExtendedListCHAR = "l";
var extendedListReg = /^l(\d+)$/;
var REPEAT_NO_LIMIT = -1;
var REPEAT_RESTART = 0;
var OPTION_ITEM_REPEAT = false; // should be OPTION_LIST_REPEAT=!OPTION_ITEM_REPEAT, But is not needed
var OPTION_LIST_REPEAT = true; // should be OPTION_LIST_REPEAT=!OPTION_ITEM_REPEAT, But is not needed
var DEFAULT_ZONELIST_ITEM = [PRODSTOP, 0, 0, false, false, REPEAT_NO_LIMIT]; // ["prodId","AmntToDo","AmntToGo","rackMode","mode","AmntToRepeat"] //mode=Field,Rack,Time,Repeat
var DEFAULT_ZONELIST_ITEM_ARRAY = [DEFAULT_ZONELIST_ITEM];
var DEFAULT_ZONELIST_ITEM_ARRAY2 = [DEFAULT_ZONELIST_ITEM, DEFAULT_ZONELIST_ITEM];
var DEFAULT_ZONELIST_MILL = [PRODSTOP, 0, 0, 0, []]; // {"pId":PRODSTOP,"AmntTodo":0,"AmntMax":0,"AmntItem":[]}
var DEFAULT_ZONELIST_MILL_ARRAY = [DEFAULT_ZONELIST_MILL];
var _TEXTE_STOP = "----";
var row1 = 2;
var row2 = 21;
var row3 = 38;
...
var BUILDING2FEED = [, , [1, 2], [3, 4], [5, 6], [7, 8], , [9, 21], [10, 110], [11, 151], [12], [92, 93], [108, 109], , , [153, 154]]; // task_new_product
var FEEDBONUSTIME = { "1": 600, "2": 1200, "3": 900, "4": 1800, "5": 2400, "6": 4800, "7": 4800, "8": 9600, "92": 14400, "93": 28800, "108": 3600, "109": 1800, "153": 1560, "154": 3120 }; //in sek  // task_new_product
var CROPCOST = { "13": 2.5, "14": 180, "15": 30, "16": 500 };
var linecount = 0;
...
handled.set = function (zoneNrS) {
    try {
        ...
        GM_logError("handled.set", "zoneNrS=" + zoneNrS, "", err);
    }
};
var queueItemBoxWidth = 58;
var queueItemBoxTimeHeight = 28;
var STEPPINGRACK = 100;
var STEPPINGFIELD = 1;
...
//const CLOSETIME=180;
var RAISETIME = 2000; //milliseconds
var OpenWindow = { "all": ["multiframe", "divQueueBox", "divChooseBox", "infoPanel", "friendscontainer", "guildmaincontainer", "createguildbox", "reallocateBuildingContainer", "messages_main"],
...
var botArbiter = new function () {
    ...
    this.add = function (event) {
        try {
            if (LOGGING_ARBITER) {
                GM_logInfo("botArbiter.add", "event=" + event, "bot.isActive()=" + bot.isActive(), "Begin");
            }
            var priority, fkt = null;
            switch (event) {
                case "sessionEnds":
                    priority = 999;
                    fkt = doGameSessionEnds;
                    break;
                case "megafield":
                    priority = 35;
                    fkt = autoMegafield;
                    break;
                case "otherAccMegafieldReady":
                    priority = 34;
                    fkt = doGameOtherAccReady;
                    break;
                case "farm":
                    priority = 30;
                    fkt = autoFarm;
                ...
                case "foodworld":
                    ...
                    fkt = autoFoodworld;
                    break;
                case "clothingDonation":
                    priority = 5;
                    fkt = autoClothingDonation;
                    break;
                case "donkey":
                    priority = 5;
                    fkt = autoDonkey;
                ...
                case "buyPetsParts":
                    priority = 5;
                    fkt = autobuyPetsParts;
                    break;
                case "activatePowerUp":
                    priority = 5;
                    fkt = autoActivatePowerUp;
                    break;
                case "farmi":
                    priority = 5;
                    fkt = autoFarmi;
                ...
                case "goOlympiaRun":
                    ...
                    fkt = autoOlympiaRun;
                    break;
                case "goSiebenSchlaefer":
                    priority = 5;
                    fkt = autoSiebenSchlaeferRun;
                    break;
                case "goIceDelivery":
                    priority = 5;
                    fkt = autoIceDelivery;
            ...
            GM_logError("botArbiter.add", "event=" + event, "", err);
        }
    };
    this.clear = function (event) {
        try {
            if (LOGGING_ARBITER) {
                GM_logInfo("botArbiter.clear", "event=" + event, "intervalBusy=" + intervalBusy, "Begin");
            }
            if (stack[event]) {
                if (intervalBusy) {
            ...
            if (LOGGING_ARBITER) {
                GM_logInfo("botArbiter.clear", "", "stack=" + implode(stack, "botArbiter.clear/stack"), "End");
            }
        }
        catch (err) {
            GM_logError("botArbiter.clear", "event=" + event, "", err);
        }
    };
    this.run = function () {
        try {
            ...
            try {
                ...
                GM_logError("botArbiter.run", "", "", err);
            }
            intervalBusy = false;
        }
        catch (err) {
    ...
    this.start = function () {
        try {
            ...
            GM_logError("botArbiter.start", "", "", err);
        }
    };
    this.stop = function () {
        try {
            if (LOGGING_ARBITER) {
                GM_logInfo("botArbiter.stop", "", "bot.isActive()=" + bot.isActive(), "Begin");
            }
            if (intervalBusy) {
            ...
            GM_logError("botArbiter.stop", "", "", err);
        }
    };
    this.check = function () {
        try {
            if (LOGGING_ARBITER) {
                GM_logInfo("botArbiter.check", "", "", "Begin");
            }
            ...
            if (bot.isActive()) {
                ...
                if ($("divGoToClothingDonation") && !zoneWaiting["clothingDonation"]) {
                    ...
                    if (!log && (settings.get("account", "botUseClothingGamble") || settings.get("account", "botUseClothingDonation")) || // No log available (and at least one option is checked) => Let bot open the dialog
                        ...
                        settings.get("account", "botUseClothingDonation") && log["gambleInfo"][0]["gain"] > 0) {
                ...
                if ((settings.get("account", "botUseOlympiaRun")) && (isSchokolade)) {
                ...
                if (cell) {
                    if (settings.get("account", "botPreferMegafield") && cell.getAttribute("ismegafield") === "true") {
            ...
            if (LOGGING_ARBITER) {
                GM_logInfo("botArbiter.check", "", "", "End");
            }
        }
        catch (err) {
            GM_logError("botArbiter.check", "", "", err);
        }
    };
};
    ...
    try {
        ...
        this.deactivate = function () {
            try {
                ...
                GM_logError("bot.deactivate", "", "", err);
            }
        };
        this.start = function () {
            try {
        ...
        this.run = function () {
            try {
                ...
                if (divInfo.innerHTML != "Busy: " + action) {
                    divInfo.innerHTML = "Busy: " + action;
                }
                if (!busy) {
                    try {
                        window.clearInterval(interval);
        ...
        this.setAction = function (str) {
            try {
                ...
                GM_logError("bot.setAction", "str=" + str, "", err);
            }
        };
        this.checkRun = function (type, run, noBot) {
            try {
                ...
                GM_logError("bot.checkRun", "type=" + type, "", err);
            }
        };
    }
    catch (err) {
...
var settings = new function () {
    ...
    this.load = function (scope) {
        try {
            ...
            switch (scope) {
                ...
                case "country":
                    if (COUNTRY) {
                        key = COUNTRY + "_settings";
                    }
                    break;
                case "server":
                    if (COUNTRY && SERVER) {
                        key = COUNTRY + "_" + SERVER + "_settings";
                    }
                    break;
                case "account":
                    if (COUNTRY && SERVER && USERNAME) {
                        key = COUNTRY + "_" + SERVER + "_" + USERNAME + "_settings";
                    }
                    break;
            }
            if (key) {
            ...
            GM_logError("settings.load", "scope=" + scope, "", err);
        }
    };
    this.save = function (scope) {
        try {
            ...
            GM_logError("settings.save", "scope=" + scope, "", err);
        }
    };
    this.get = function (scope, key) {
        try {
            ...
            GM_logError("settings.get", "scope=" + scope + " key=" + key, "", err);
        }
    };
    this.set = function (scope, key, value) {
        try {
            ...
            GM_logError("settings.set", "scope=" + scope + " key=" + key, "", err);
        }
    };
    this.reset = function (scope, key) {
        try {
            ...
            GM_logError("settings.reset", "scope=" + scope + " key=" + key, "", err);
        }
    };
        ...
        try {
            if (require[scope][key]) {
                for (var j = require[scope][key].length - 1; j >= 0; j--) {
                    if (!settings.get(require[scope][key][j][0], require[scope][key][j][1])) {
                        return false;
                    }
                }
            }
            ...
            GM_logError("settings.checkRequire", "scope=" + scope + " key=" + key, "", err);
        }
    };
    // Special functions
...
function startCloseWindowTimer() {
    try {
        ...
        if (divCWL.style.display != "block") {
            divCWL.style.display = "block";
        }
        var divCW = $("divCloseWindow");
        if (divCW.style.display != "block") {
            divCW.style.display = "block";
        }
        var cell;
        if (!bot.isBusy()) {
            if (--closeWindowTime > 0 && !checkOpenWindow(true)) {
                ...
                if (!closeWindowTimer)
                    closeWindowTimer = window.setInterval(startCloseWindowTimer, 1000);
            }
            else {
                ...
                else if ((cell = $("wbwcontainer")) && (cell.style.display == "block")) {
        ...
        GM_logError("startCloseWindowTimer", "", "", err);
    }
}
    ...
    try {
        ...
        if (automatStarted && !bot.isBusy()) {
            ...
            for (var ctn in OpenWindow) {
                if (!OpenWindow.hasOwnProperty(ctn)) {
                    continue;
                }
            ...
            else if (!closeWindowTimer && !justcheck) {
        ...
        GM_logError("checkOpenWindow", "justcheck=" + justcheck, "ctn=" + ctn + " wn=" + wn, err);
    }
}
...
function calcDauer(dauer, bonus) {
    var gesamtdauer = 0;
    while (dauer > 0) {
        dauer *= bonus;
...
function calcProductScore(product, zoneNrF, amount, endtime) {
    ...
    switch (getBuildingTyp(zoneNrF)) {
        ...
        case "forest":
            if (amount == null || amount === undefined) {
                amount = calcProductPositions(product, zoneNrF);
            }
            return unsafeData.prodPoints[1][product] * amount;
            break;
        case 1:
        ...
        case 2:
            if (amount == null || amount === undefined) {
                amount = unsafeData.zones.getAnimals(zoneNrF);
            }
...
function calcProductPositions(product, zoneNrF) {
    try {
        ...
        GM_logError("calcProductPositions", "product=" + product + " zoneNrF=" + zoneNrF, "", err);
    }
}
        ...
        if (tempZoneProductionData) {
            if (tempZoneProductionData[0][0][product]) {
                for (var i in tempZoneProductionData[0][0][product]) {
                    if (!tempZoneProductionData[0][0][product].hasOwnProperty(i)) {
                        continue;
                    }
                ...
                for (var iProdStr in zoneErnteCurr[iTypeStr]) {
                    ...
                    for (var i = 0; i <= zoneErnteCurr[0][iTypeStr][iProdStr].length(); i++) {
                        if ((t1 <= zoneErnteCurr[0][iTypeStr][iProdStr][i][2]) && (zoneErnteCurr[0][iTypeStr][iProdStr][i][2] <= t2)) {
                            if (!production[iTypeStr]) {
                                production[iTypeStr] = {};
                            }
                            if (!production[iTypeStr][iProdStr]) {
                                production[iTypeStr][iProdStr] = 0;
                            }
    ...
    if (extendedListReg.test(zoneNr)) {
        ...
        switch (zoneNr) {
            ...
            case "foodworld-4":
                ...
                break;
            case "farmersmarket-0":
            case "farmersmarket-1":
            case "farmersmarket-2":
            case "farmersmarket-3":
            case "farmersmarket-4":
            case "farmersmarket-5":
            case "farmersmarket-6":
            case "farmersmarket-7":
    ...
    try {
        ...
        if ("megafield" == zoneNrL) {
            ...
            for (var fz in unsafeData.ALL_SLOTS) {
                if (!unsafeData.ALL_SLOTS.hasOwnProperty(fz)) {
                    continue;
                }
                help = unsafeData.zones.getEndtime(fz);
                zoneTimes[fz] = ((help == NEVER) ? now : help);
...
function getZoneActiveList() {
    ...
    for (var zoneNrF in zoneList) {
        if (!zoneList.hasOwnProperty(zoneNrF)) {
            continue;
        }
        zones[zoneList[zoneNrF]] = 1;
    }
    for (var zoneNrL in zones) {
        if (!zones.hasOwnProperty(zoneNrL)) {
            continue;
        }
...
function getZonesFromList(zoneNrL) {
    ...
    if ("megafield" == zoneNrL) {
        ...
        for (var zoneNrS in zoneToList) {
            if (!zoneToList.hasOwnProperty(zoneNrS)) {
                continue;
            }
...
function createZoneList(zoneNrS) {
    switch (getZoneType(zoneNrS)) {
        case "windmill":
            zoneList[zoneNrS] = DEFAULT_ZONELIST_MILL_ARRAY.clone();
            break;
        case 2:
            zoneList[zoneNrS] = DEFAULT_ZONELIST_ITEM_ARRAY2.clone();
            break;
...
function getZoneListId(zoneNrF, slot) {
    try {
        ...
        if (!zoneToList[zoneToListId]) {
            setZoneListId(zoneToListId, zoneToListId);
        }
        if (!zoneList[zoneToList[zoneToListId]]) {
            createZoneList(zoneToList[zoneToListId]);
        }
        return zoneToList[zoneToListId];
    }
    catch (err) {
        GM_logError("getZoneListId", "zoneNrF=" + zoneNrF + " slot=" + slot, "", err);
    }
}
function setZoneListId(zoneNrF, zoneNrL) {
    try {
        if (!unsafeData.ALL_SLOTS[zoneNrF]) {
            return false;
        }
        if ((!extendedListReg.test(zoneNrL)) && (!unsafeData.ALL_SLOTS[zoneNrL])) {
            return false;
        }
        ...
        GM_logError("setZoneListId", "zoneNrF=" + zoneNrF + " zoneNrL=" + zoneNrL, "", err);
    }
}
function deleteZoneInList(zoneNrL) {
    try {
        ...
        for (var fz in zoneToList) {
            if (!zoneToList.hasOwnProperty(fz)) {
                continue;
            }
            if (zoneToList[fz] == zoneNrL) {
                setZoneListId(fz, fz);
            }
        }
        ...
        GM_logError("deleteZoneInList", "zoneNrL=" + zoneNrL, "", err);
    }
}
function addZoneToList() {
    try {
        ...
        for (var lz in zoneList) {
            if (!zoneList.hasOwnProperty(lz)) {
                continue;
            }
            if (extendedListReg.test(lz)) {
                if (extendedListReg.exec(lz)[1] != newZone)
                    break;
                newZone++;
            }
        }
...
function getFarmZone(gardenNr, farm) {
    ...
    if (isNaN(gardenNr)) {
        ...
        return parseInt(gardenNr, 10) + 6 * (parseInt(farm, 10) - 1);
    }
}
...
function getZoneStockType(zoneNrFL) {
    var fzType = getBuildingTyp(zoneNrFL);
    if (extendedListReg.test(zoneNrFL)) {
        return 0;
    }
    else {
        switch (fzType) {
...
function getBuildingTyp(zoneNr) {
    try {
        ...
        GM_logError("getBuildingTyp", "zoneNr=" + zoneNr, "", err);
    }
}
        ...
        if ((extendedList = extendedListReg.exec(zoneNrL)) != null) {
            ...
            if (showLocation) {
                ...
                for (var i = 0; i < zones.length; i++) {
                    ...
                    if (zones[i][0] == zoneNrF)
                        txt += "</span>";
                    if (zones[i][0] == zoneNrL)
                        txt += "<u>";
                ...
                txt += "</span>";
                if (settings.get("account", "useQueueList") && queueNum != null && queueNum >= 0)
                    txt += "&nbsp;{" + queueNum + "}";
                zones = null;
                lz = null;
            }
            ...
            switch (getBuildingTyp(zoneNrF)) {
                case 1:
                    if (settings.get("account", "useQueueList") && showLocation && queueNum != null && queueNum >= 0) {
                        txt = "&nbsp;{" + queueNum + "}";
                    }
                    if (showBonus) {
                        var currBonus = unsafeData.zones.getBonus(zoneNrF);
...
function drawAutomatIcon(name, zoneNrS, appendTo, style) {
    try {
        ...
        if (!(automatIcons[name] && automatIcons[name][1])) {
            ...
            automatIcons[name][1].addEventListener("click", function (event) {
                try {
                    ...
                    GM_logError("automatIcon.click", "$('" + this.id + "') zoneNrF=" + zoneNrF + " zoneNrS=" + zoneNrS + " zoneNrL=" + zoneNrL, "", err);
                }
            }, false);
        ...
        GM_logError("drawAutomatIcon", "zoneNrS=" + zoneNrS, "", err);
    }
}
    ...
    try {
        ...
        if (OPTION_LIST_REPEAT && zoneSettings[zoneNrL]["repeat"] != undefined) {
            ...
            newelm.addEventListener("click", function () {
                ...
                if (!zoneSettings[zoneNrL]["repeat"]) {
                    for (var i = 0; i < zoneList[zoneNrL].length; i++)
                        zoneList[zoneNrL][i][2] = 0;
        ...
        if (fzType == "windmill") {
            ...
            newelm.addEventListener("click", function () {
                ...
                for (var iProd in autoMillStorage) {
                    if (!autoMillStorage.hasOwnProperty(iProd)) {
                        continue;
                    }
                ...
                if (zoneList[zoneNrL].length <= 0)
                    zoneList[zoneNrL] = DEFAULT_ZONELIST_MILL_ARRAY.clone();
        ...
        if (!isNaN(zoneNrS)) {
            ...
            newelm.addEventListener("click", function () {
                ...
                newdiv.addEventListener("click", function (event) {
                    ...
                    if (reAppendTo.indexOf("tdAutoMatOverview") == 0) {
        ...
        GM_logError("drawQueueBox", "zoneNrS=" + zoneNrS + " zoneNrL=" + zoneNrL + " appendTo.id=" + appendTo.id, "", err);
    }
}
function drawQueueItemBox(zoneNrS, zoneNrL, queueNum, appendTo) {
    try {
        ...
        */
        ...
        if (OPTION_ITEM_REPEAT) {
            ...
            newelm.addEventListener("change", function () {
                try {
                    ...
                    GM_logError("drawQueueItemBox InputRepeatField changed", "", "", err);
                }
            }, false);
        }
        // Input Field
        ...
        newelm.addEventListener("change", function () {
            try {
                ...
                GM_logError("drawQueueItemBox InputField changed", "", "", err);
            }
        }, false);
        /*
        ...
        */
        ...
        GM_logError("drawQueueItemBox", "zoneNrS=" + zoneNrS + " zoneNrL=" + zoneNrL + " queueNum=" + queueNum + " appendTo.id=" + appendTo.id, "", err);
    }
}
function drawQueueListSmall(zoneNrF, zoneNrL, appendTo) {
    if (zoneNrL === undefined || zoneNrL == null)
        zoneNrL = getZoneListId(zoneNrF);
    appendTo.style.display = "none";
    appendTo.innerHTML = "";
    ...
    if (false && fzType == 3) {
        ...
        for (queueNum = 0; queueNum < zoneList[zoneNrL].length; queueNum++) {
            if (!settings.get("account", "useQueueList") && queueNum > 0 && getBuildingTyp(zoneNrL) == 1)
                break;
            if (fzType == "windmill") {
...
function drawQueueChooseItemBox(zoneNrS, zoneNrL, queueNum, appendTo) {
    try {
        ...
        for (var iProd = 0; iProd < unsafeData.prodName[0].length; iProd++) {
            ...
            if ((unsafeData.PRODUCT2BUILDING[0][iProd] == fzZoneType) && !(unsafeData.prodBlock[0][iProd] && unsafeData.prodBlock[0][iProd].match(/[uvlq]/))) {
                ...
                if (zoneNrS == 25 || zoneNrS == 26 || zoneNrS == 27 || zoneNrS == 28 || zoneNrS == 29 || zoneNrS == 30) {
                    if ((unsafeData.prodTyp[0][iProd] != "ex")) {
                        continue;
                    }
                }
                else {
                ...
                if (zoneNrS == 31 || zoneNrS == 32 || zoneNrS == 33 || zoneNrS == 34 || zoneNrS == 35 || zoneNrS == 36) {
                    if ((unsafeData.prodTyp[0][iProd] != "alpin")) {
                        continue;
                    }
                }
                else {
                ...
                if (zoneNrS == 25 || zoneNrS == 26 || zoneNrS == 27 || zoneNrS == 28 || zoneNrS == 29 || zoneNrS == 30) {
                    ...
                    //} else if (unsafeWindow.farm==6) {
                }
                else if (zoneNrS == 31 || zoneNrS == 32 || zoneNrS == 33 || zoneNrS == 34 || zoneNrS == 35 || zoneNrS == 36) {
        ...
        */
        ...
        GM_logError("drawQueueChooseItemBox ", "", "", err);
    }
}
function drawFactoryChooseItemBox(zoneNrS, zoneNrL, appendTo) {
    try {
        ...
        for (var v in unsafeData.BUILDING_INPUT[getZoneType(zoneNrS)]) {
            if (!unsafeData.BUILDING_INPUT[getZoneType(zoneNrS)].hasOwnProperty(v)) {
                continue;
            }
            if (unsafeData.prodBlock[0][v]) {
                continue;
            }
            newdiv = createElement("div", { "class": "divChooseItem link v" + v, "id": "divChooseItem" + zoneNrL + "Q0I" + v }, appendTo);
            if (zoneList[zoneNrL][0][0] == v) {
                newdiv.style.border = "2px solid black";
            }
            newdiv.addEventListener("click", function () {
                var zoneNrF = this.parentNode.getAttribute("zoneNrF");
        ...
        GM_logError("drawFactoryChooseItemBox ", "", "", err);
    }
}
function drawFuelstationChooseItemBox(zoneNrS, zoneNrL, appendTo) {
    try {
        ...
        for (var i in zoneToList) {
            if (!zoneToList.hasOwnProperty(i)) {
                continue;
            }
        ...
        for (var l = 0; l < level; l++) {
            for (var v = 0; v < unsafeData.FUELSTATION_INPUT[l].length; v++) {
                if (unsafeData.prodBlock[0][unsafeData.FUELSTATION_INPUT[l][v][0]]) {
                    continue;
                }
                ...
                if (zoneList[zoneNrL][0][0] == unsafeData.FUELSTATION_INPUT[l][v][0]) {
                    newdiv.style.border = "2px solid black";
                }
                newdiv.addEventListener("click", function () {
                    var zoneNrF = this.parentNode.getAttribute("zoneNrF");
    ...
    try {
        ...
        for (var i = 0; i < item.objects[Object.keys(item.objects)[zoneNrF_h[1] - 1]].length; i++) {
            ...
            if (!(item.objects[Object.keys(item.objects)[zoneNrF_h[1] - 1]][i].hasOwnProperty("buyable"))
                ...
                if (zoneList[zoneNrL][0][0] == parseInt(item.objects[Object.keys(item.objects)[zoneNrF_h[1] - 1]][i].oid)) {
                ...
                newdiv.addEventListener("click", function () {
                    ...
                    zoneList[zoneNrL] = DEFAULT_ZONELIST_ITEM_ARRAY.clone();
                    zoneList[zoneNrL][0][0] = parseInt(/Q0I(.*)$/.exec(this.id)[1], 10);
                    if (this.parentNode.parentNode == $("divChooseBoxInner")) {
                        click($("divChooseBoxClose"));
                    }
                    updateQueueBox(zoneNrF, zoneNrL);
                }, false);
...
function drawStableChooseFeedBox(zoneNrF, zoneNrL, appendTo) {
    try {
        ...
        if (zoneList[zoneNrL].length != 2) {
            ...
            if (zoneList[zoneNrL][1][0] == PRODSTOP) {
                zoneList[zoneNrL][1] = DEFAULT_ZONELIST_ITEM.clone();
            }
        }
        var feedSort = [,];
        var feedAmount = [0, 0];
        ...
        if (unsafeWindow.premium == 0 && level > 9) {
            growTime -= 1;
        } // small hack. non-premiums cant feed completely
            ...
            if (feedAmount[0] + feedAmount[1] == 0) {
                ...
                if (feedSort[index]) {
                    if (feedAmount[index] > 0) {
                        if (feedAmount[(index + 1) % 2] > 0) {
                            calcTime++;
                        } // a bit time is lost when 2 feeds are used
        ...
        for (var k = 0; k < BUILDING2FEED[getZoneType(zoneNrF)].length; k++) {
            ...
            if (!(unsafeData.prodBlock[0][iProd] && unsafeData.prodBlock[0][iProd].match(/l/))) {
                ...
                newdiv1.addEventListener("click", function (event) {
                    ...
                    if (!isNaN(help)) {
                        calcStableChooseFeedBox(parseInt(this.parentNode.getAttribute("name"), 10), help);
                    }
                }, false);
        ...
        GM_logError("drawStableChooseFeedBox ", "", "", err);
    }
}
function drawMillChooseItemBox(zoneNrF, zoneNrL, queueNum, appendTo) {
    try {
        ...
        for (var iProd in autoMillStorage) {
            ...
            if ((autoMillStorage[iProd][0] - autoMillStorage[iProd][1]) > 0) {
                ...
                newdiv.addEventListener("click", function () {
                    ...
                    if (zoneList[zoneNrL][queueNum][0] != recipe)
                        zoneList[zoneNrL][queueNum] = [recipe, 1, 0, 0, []];
                    toolTip.hide(this);
                    if (this.parentNode == $("divChooseBoxInner"))
                        click($("divChooseBoxClose"));
                    updateQueueBox(zoneNrF, zoneNrL);
        ...
        GM_logError("drawMillChooseItemBox ", "", "", err);
    }
}
function drawForestryChooseItemBox(zoneNrF, zoneNrL, queueNum, appendTo) {
    try {
        ...
        for (var iProd in unsafeData.prodName[1]) {
            if (!unsafeData.prodName[1].hasOwnProperty(iProd)) {
                continue;
            }
            if (unsafeData.prodTyp[1][iProd] != zoneStockTyp) {
                continue;
            }
            ...
            if (!(unsafeData.prodBlock[1][iProd] && unsafeData.prodBlock[1][iProd].match(/q/))) {
                ...
                for (var i in unsafeData.prodRequire[1][iProd]) {
                    if (!unsafeData.prodRequire[1][iProd].hasOwnProperty(i)) {
                        continue;
                    }
        ...
        GM_logError("drawForestryChooseItemBox", "iProd:" + iProd, "", err);
    }
}
function drawFoodworldChooseItemBox(zoneNrS, zoneNrL, queueNum, appendTo) {
    try {
        ...
        for (var iProd = 0; iProd < unsafeData.prodName[0].length; iProd++) {
            if (fzZoneType == unsafeData.PRODUCT2BUILDING[0][iProd]) {
        ...
        GM_logError("drawFoodworldChooseItemBox ", "", "", err);
    }
}
function drawAnimalBreedingChooseItemBox(zoneNrS, zoneNrL, queueNum, appendTo) {
    try {
        ...
        GM_logError("drawAnimalBreedingChooseItemBox ", "", "", err);
    }
}
function drawChooseItemBoxPony(zoneNrS, zoneNrL, queueNum, appendTo) {
    try {
        ...
        GM_logError("drawChooseItemBoxPony ", "", "", err);
    }
}
function drawChooseItemBoxMegafield(zoneNrS, zoneNrL, queueNum, appendTo) {
    try {
        ...
        GM_logError("drawChooseItemBoxMegafield ", "", "", err);
    }
}
function updateQueueBox(zoneNrS, zoneNrL) {
    try {
        ...
        for (var i in automatIcons) {
            if (!automatIcons.hasOwnProperty(i)) {
                continue;
            }
        ...
        if ($("divAutoMatQueueBox" + zoneNrL)) {
            ...
            loop: for (queueNum = 0; queueNum < zoneList[zoneNrL].length; queueNum++) {
                ...
                var fz;
                if (!newProdStock[iType][iProd]) {
                    newProdStock[iType][iProd] = 0;
                }
                ...
                if (settings.get("account", "showQueueTime")) {
                    ...
                    if (fzWindmill) {
                        ...
                        if (!timesArray[zoneNrS])
                            timesArray[zoneNrS] = new Array();
                        iTogoArray[zoneNrS] = 0;
                        for (var k = 0; k < iTogo; k++) {
                            zoneTimes[zoneNrS] += timeArray[zoneNrS];
                            if (timesArray[zoneNrS].length <= 10) {
                                timesArray[zoneNrS].push(zoneTimes[zoneNrS]);
                            }
                            iTogoArray[zoneNrS]++;
                        }
                        endTime = zoneTimes[zoneNrS];
                    }
                    else if ((iTot <= 0) || iStop) {
                        for (fz in zoneTimes) {
                            if (!zoneTimes.hasOwnProperty(fz)) {
                                continue;
                            }
                            if (isNaN(iTogoArray[fz]))
                                iTogoArray[fz] = 0;
                            iTogoArray[fz]++;
                            if (!beginTime[fz])
                                beginTime[fz] = zoneTimes[fz];
                            if (!timeArray[fz])
                                timeArray[fz] = calcProductionTime(iProd, getZoneNameCorrected(fz));
                        }
                    }
                    else if (iRackMode) {
                        while ((newRackAmount[iProd] + iTogoAmount) < iTot) {
                            ...
                            if (!beginTime[fz])
                                beginTime[fz] = zoneTimes[fz];
                            if (!timeArray[fz])
                                timeArray[fz] = calcProductionTime(iProd, cfz);
                            if (isNaN(iTogoArray[fz]))
                                iTogoArray[fz] = 0;
                            iTogoArray[fz]++;
                            ...
                            if (timesArray[fz].length <= 10)
                                timesArray[fz][timesArray[fz].length] = zoneTimes[fz];
                            ...
                            iTogo++;
                            if (endTime < zoneTimes[fz])
                                endTime = zoneTimes[fz];
                        }
                    }
                    else {
                        for (var k = 0; k < iTogo; k++) {
                            ...
                            for (var sType in totalCrop) {
                                if (!totalCrop.hasOwnProperty(sType)) {
                                    continue;
                                }
                                if (!newProdStock[sType]) {
                                    newProdStock[sType] = {};
                                }
                                for (var sProd in totalCrop[sType]) {
                                    if (!totalCrop[sType].hasOwnProperty(sProd)) {
                                        continue;
                                    }
                                    if (!newProdStock[sType][sProd]) {
                                        newProdStock[sType][sProd] = 0;
                                    }
                            ...
                            if (unsafeData.prodRequire[iType] && unsafeData.prodRequire[iType][iProd]) {
                                for (var i in unsafeData.prodRequire[iType][iProd]) {
                                    if (!unsafeData.prodRequire[iType][iProd].hasOwnProperty(i)) {
                                        continue;
                                    }
                            ...
                            if (aGrowing[fz]) {
                                if (!newProdStock[iType][aGrowing[fz][0]]) {
                                    newProdStock[iType][aGrowing[fz][0]] = 0;
                                }
                            ...
                            if (!timeArray[fz]) {
                                timeArray[fz] = calcProductionTime(iProd, fz);
                            }
                            ...
                            if (!aGrowingTotal[iProd]) {
                                aGrowingTotal[iProd] = 0;
                            }
                    ...
                    if (!newRackAmount[iProd] && !fzWindmill && !fzMegafield) {
                        newRackAmount[iProd] = calcInGameProductAmount(iProd, zoneNrS, null, true);
                    }
                }
                if (!(zoneNrS == "farmersmarket-4.5" || zoneNrS == "farmersmarket-4.6" || zoneNrS == "farmersmarket-4.7")) {
        ...
        if (divChooseEndTimeCurr) {
            ...
            for (var i = 0; i <= queueNum; i++) {
                if (foundStop = zoneList[zoneNrL][i][0] == PRODSTOP)
                    break;
            }
            ...
            switch (fzType) {
                ...
                case 3: {
                    ...
                    for (var v in unsafeData.BUILDING_INPUT[fzZoneType]) {
                        if (!unsafeData.BUILDING_INPUT[fzZoneType].hasOwnProperty(v)) {
                            continue;
                        }
                        if (unsafeData.prodBlock[0][v]) {
                            continue;
        ...
        GM_logError("updateQueueBox", "zoneNrS=" + zoneNrS + " zoneNrL=" + zoneNrL, "err_trace=" + err_trace, err);
    }
}
function redrawQueueBox(zoneNrS, zoneNrL, appendTo) {
    ...
    if (zoneNrL === undefined || zoneNrL == null)
        zoneNrL = getZoneListId(zoneNrS);
    if ($("divChooseBox").style.display == "block")
        click($("divChooseBoxClose"));
...
function toolTipProductSmall(zoneNrS, zoneNrL, queueNum, parent) {
    try {
        ...
        if (isIcon) {
            ...
            if (settings.get("account", "showQueueTime"))
                var zoneBeginTime = explode(parent.getAttribute("zoneBeginTime"), "toopTipProductSmall/[0]/zoneBeginTime", {});
        }
        else if (isTimeLine) {
            ...
            if (settings.get("account", "showQueueTime"))
                var zoneBeginTime = explode(parent.parentNode.parentNode.getAttribute("zoneBeginTime"), "zoneBeginTimeStable");
            ...
            if (settings.get("account", "showQueueTime"))
                var zoneBeginTime = explode(parent.parentNode.getAttribute("zoneBeginTime"), "toopTipProductSmall/[2]/zoneBeginTime", {});
        }
        else if (fzForestry) {
            var iProd = parseInt(parent.getAttribute("class").replace("divChooseItem link f_symbol", ""), 10);
            if (settings.get("account", "showQueueTime"))
                var zoneBeginTime = explode(parent.parentNode.getAttribute("zoneBeginTime"), "toopTipProductSmall/[5]/zoneBeginTime", {});
        }
        else if (fzPowerUp) {
            var iProd = parseInt(parent.getAttribute("class").replace("divChooseItem link r", ""), 10);
            if (settings.get("account", "showQueueTime"))
                var zoneBeginTime = explode(parent.parentNode.getAttribute("zoneBeginTime"), "toopTipProductSmall/[3]/zoneBeginTime", {});
        }
        else if (fzPony) {
            if (settings.get("account", "showQueueTime")) {
        ...
        var productTime;
        if (!iStop) {
            productTime = calcProductionTime(iProd, fz);
        }
        ...
        if (fzPowerUp) {
            ...
            if (iStop) {
                ...
                if ((fzType != 2) || isIcon || isTimeLine) {
                    ...
                    createElement("div", { "class": "tableSeperater" }, content);
                    createElement("div", { "class": "tableSeperater" }, content);
                    newrow = createElement("div", { style: "display:table-row;width:100%;" }, content);
                ...
                if (fzType == 2 && (isIcon || isTimeLine)) {
                    ...
                    for (var i in unsafeData.prodRequire[0][iProd]) {
                        if (!unsafeData.prodRequire[0][iProd].hasOwnProperty(i)) {
                            continue;
                        }
            ...
            if (settings.get("account", "showQueueTime") && !isTimeLine && lShowTime && (!iStop || foundStop)) {
                if (lZonesAmount > 1 || fzType == "sawmill") {
                    ...
                    for (var fz in zoneBeginTime) {
                        if (!zoneBeginTime.hasOwnProperty(fz)) {
                            continue;
                        }
...
function toolTipProduct(zoneNrF, zoneNrL, queueNum, parent) {
    try {
        ...
        var isSelfConsumingType = (["v", "fl", "hr", "ex", "alpin"].indexOf(unsafeData.prodTyp[0][iProd]) >= 0);
        var content = createElement("div");
        ...
        if (iStop) {
            ...
            if ((lRepeat || iRepeat || lShuffle || iRackMode) && iTogo <= 0) {
                ...
                if (!extendedListReg.test(zoneNrL)) {
                    if (!iRackMode) {
                        ...
                        if ((lRepeat || iRepeat || lShuffle) && iDone > 0) {
                            var newspan = createElement("div", { style: "display:table-row;" }, content);
            ...
            if (foundStop) {
                ...
                if (iTogo > 0) {
                    ...
                    for (var fz in timesArray) {
                        if (!timesArray.hasOwnProperty(fz)) {
                            continue;
                        }
                    ...
                    for (var fz in timesArray) {
                        ...
                        if (timesCount >= 11)
                            createElement("div", { style: "display:table-cell;padding:0px 2px;" }, newspan, timesArray[fz].length >= 11 ? "&nbsp;....&nbsp;" : "");
                        if (!iLastInf) {
    ...
    try {
        ...
        if (iStop) {
            ...
            if (unsafeWindow.formulas[0][iProd]) {
                ...
                for (var bonus = 0; bonus < info[5].length; bonus++) {
                    if (!!info[5][bonus])
                        break;
                }
                if (bonus != undefined) {
                ...
                for (var j = 0; j < info[3].length; j++) {
                    if (info[3][j]) {
                        ...
                        else if (zoneList[zoneNrL][queueNum][3] < 0) {
...
function setNextQueueItem(zoneNrS) {
    try {
        ...
        var zoneNrL = getZoneListId(zoneNrS);
        if (isNaN(zoneList[zoneNrL][0][1])) {
            zoneList[zoneNrL][0] = DEFAULT_ZONELIST_ITEM.clone();
        }
        if (isNaN(zoneList[zoneNrL][0][2])) {
            zoneList[zoneNrL][0][2] = 0;
        }
    ...
    try {
        ...
        if ((settings.get("account", "useQueueList") || isNaN(zoneNrF)) && (zoneList[zoneNrL][0][0] != PRODSTOP) && (nextItemMode || (((zoneList[zoneNrL][0][3] && calcInGameProductAmount(zoneList[zoneNrL][0][0], zoneNrF, null, true) < zoneList[zoneNrL][0][1]) || (!zoneList[zoneNrL][0][3] && zoneList[zoneNrL][0][1] <= ((OPTION_ITEM_REPEAT && zoneList[zoneNrL][0][4]) || (OPTION_LIST_REPEAT && zoneSettings[zoneNrL]["repeat"]) ? zoneList[zoneNrL][0][2] : 0)))))) {
            ...
            do {
                ...
                if ((OPTION_ITEM_REPEAT && zoneList[zoneNrL][0][4]) || (OPTION_LIST_REPEAT && zoneSettings[zoneNrL]["repeat"])) {
                    if (zoneList[zoneNrL][0][3] && !iDoRack && !zoneSettings[zoneNrL]["shuffle"]) {
                        ...
                        if (!zoneSettings[zoneNrL]["shuffle"]) {
                            ...
                            if (zoneList[zoneNrL][0][5] != REPEAT_NO_LIMIT && OPTION_ITEM_REPEAT && zoneList[zoneNrL][0][4]) {
                                zoneList[zoneNrL][0][5]--;
                            }
                        }
        ...
        itemsRackDone = null;
        itemsRepeatDone = null;
        itemsStopsFound = null;
        iDoRack = null;
        reFillQueueBox(zoneNrF);
    ...
    try {
        ...
        do {
            ...
            if (zoneList[zoneNrL][0][0] == PRODSTOP) {
                ...
                if (pCount == zoneList[zoneNrL].length) {
        ...
        GM_logError("reSortWindmill ", "", "", err);
    }
}
...
function reCalculateWindmill() {
    try {
        ...
        for (var rId in autoMillStorage) {
            if (!autoMillStorage.hasOwnProperty(rId)) {
                continue;
            }
        ...
        for (var v = 0; v < zoneList[zoneNrL].length; v++) {
            ...
            if (rId != PRODSTOP) {
                if (autoMillStorage[rId]) {
                    ...
                    if (autoMillStorage[rId][1] > autoMillStorage[rId][0]) {
                    ...
                    for (var i = 0; i < unsafeWindow.formulas[0][rId][3].length; i++) {
                        ...
                        if (!autoMillUsedProducts[pId])
                            autoMillUsedProducts[pId] = 0;
        ...
        GM_logError("reCalculateWindmill", "", "err_trace=" + err_trace, err);
    }
}
    ...
    try {
        ...
        if (!bot.isActive()) {
            ...
            for (var fz in unsafeData.readyZone) {
                if (!unsafeData.readyZone.hasOwnProperty(fz)) {
                    continue;
                }
        ...
        GM_logError("checkReadyZone", "zoneNr=" + zoneNr, "readyZone=" + implode(unsafeData.readyZone, "checkReadyZone/readyZone"), err);
    }
}
function getReadyZone(location) {
    ...
    if (!bot.isActive()) {
        ...
        for (var zoneNrS in unsafeData.readyZone) {
            if (!unsafeData.readyZone.hasOwnProperty(zoneNrS)) {
                continue;
            }
        ...
        switch (handled.zoneBuildingTyp) {
            case 1:
                ...
                try {
                    ...
                    GM_logError("autoZoneHandle (case 1)", "", "handled.zoneNrF=" + handled.zoneNrF + " readyZone=" + implode(unsafeData.readyZone, "autoZoneHandle/readyZone"), err);
                }
                break;
            case 2:
                ...
                window.setTimeout(function (runId) {
                    try {
                        ...
                        GM_logError("autoZoneHandle (case 2*)", "", "handled.zoneNrF=" + handled.zoneNrF + " readyZone:" + implode(unsafeData.readyZone, "autoZoneHandle/readyZone"), err);
                    }
            ...
            case 3:
                ...
                try {
                    ...
                    GM_logError("autoZoneHandle (case 3)", "", "handled.zoneNrF=" + handled.zoneNrF + " readyZone:" + implode(unsafeData.readyZone, "autoZoneHandle/readyZone"), err);
                }
                break;
            case 6://Fuelstation
                break;
    ...
    try {
        ...
        switch (unsafeData.gameLocation.get()[0]) {
            case "farm":
                if ($("gardenmaincontainer").style.display == "block") {
                    ...
                    zoneTyp = unsafeData.BUILDINGTYPE[unsafeWindow.locationinfo[6]];
                }
                break;
            case "forestry":
                zoneTyp = "forest";
        ...
        switch (zoneTyp) {
            case 1:
                ...
                for (var v = 1; v <= 120; v++) {
                    // GM_log(v);
                    if (("v" == unsafeWindow.garten_kategorie[v] || "hr" == unsafeWindow.garten_kategorie[v] || "ex" == unsafeWindow.garten_kategorie[v] || "alpin" == unsafeWindow.garten_kategorie[v]) && (unsafeWindow.garten_zeit[v] != "0") && (parseInt(unsafeWindow.garten_zeit[v], 10) < NowServer)) {
                        cropable = true;
                        break;
                    }
                }
                ...
                try {
                    ...
                    for (var i = 0; i < unsafeWindow.forestry_area.length; i++) {
                        ...
                        if (unsafeWindow.forestry_area[i]['category'] == 1 && unsafeWindow.forestry_area[i]['block'] == 0 && unsafeWindow.forestry_area_time[v] != undefined && unsafeWindow.forestry_area_time[v] <= 0 && !unsafeData.zones.getProduction("forest")[2] < 25) {
                            if (parseInt(unsafeWindow.forestry_area[i]["productid"], 10) <= 5) {
                                var prodoffset = 20;
                            }
                            else {
                                var prodoffset = 19;
                    ...
                    GM_logError("autoZoneCrop (forest cropping)", "", "i=" + i + " v=" + v, err);
                }
                if (cropable) {
                    GM_logInfo("autoZoneCrop", "", "zoneTyp=" + zoneTyp, "Cropping"); // TODO text
        ...
        if (bot.checkRun("autoFarm", runId)) {
            ...
            if (zoneNrS == null) {
                ...
                for (var c = 1; c <= unsafeData.ALL_ZONES["farm"].length / 6; c++) {
                    if (unsafeWindow.farms_data.blocked[c]) {
                        continue;
                    }
...
function autoFarmCrop(runId, v) {
    try {
        if (bot.checkRun("autoFarm", runId, true)) {
            ...
            if (unsafeWindow.mode != "1") {
                ...
                if ("v" == unsafeWindow.garten_kategorie[v] || "hr" == unsafeWindow.garten_kategorie[v] || "ex" == unsafeWindow.garten_kategorie[v] || "alpin" == unsafeWindow.garten_kategorie[v]) {
                    ...
                    if ((z > 0) && (unsafeWindow.garten_x[v] == 1) && (unsafeWindow.garten_y[v] == 1)) {
        ...
        GM_logError("autoFarmCrop", "runId=" + runId, "", err);
    }
}
function autoFarmPlantInit(runId, rackitemNr) {
    try {
                ...
                if (time <= settings.get("account", "seedWaitForCrop")) {
                    ...
                    window.setTimeout(function () {
                        for (var fz in zoneWaiting) {
                            ...
                            if (zoneWaiting[fz] <= now) {
                                delete zoneWaiting[fz];
                            }
                        }
                        checkReadyZone();
        ...
        GM_logError("autoFarmPlantInit", "runId=" + runId, "", err);
    }
}
function autoFarmPlantDefault(runId) {
    try {
        ...
        GM_logError("autoFarmPlantDefault", "runId=" + runId, "", err);
    }
}
function autoFarmPlant(runId, v, didPlant, isBot) {
    try {
        ...
        if (bot.checkRun("autoFarmPlant", runId, !isBot)) {
            ...
            if (unsafeWindow.mode != "0") {
                click($("anpflanzen"));
            }
            if (v < 121) {
                if ((unsafeData.prodTyp[0][unsafeWindow.selected] == "v" || unsafeData.prodTyp[0][unsafeWindow.selected] == "hr" || unsafeData.prodTyp[0][unsafeWindow.selected] == "ex" || unsafeData.prodTyp[0][unsafeWindow.selected] == "alpin") && unsafeData.prodStock[stockNr][unsafeWindow.selected] && unsafeData.prodStock[stockNr][unsafeWindow.selected] > 0) {
                    bot.setAction("autoFarmPlant " + getText("automat_planting"));
                    if (v % 12 == 1) {
                        linecount = 0;
                    }
                    var frei = true;
                    if (unsafeWindow.garten_kategorie[v] && (((unsafeWindow.garten_kategorie[v] != "v") && (unsafeWindow.garten_kategorie[v] != "hr") && (unsafeWindow.garten_kategorie[v] != "ex") && (unsafeWindow.garten_kategorie[v] != "alpin")) || unsafeWindow.garten_zeit[v] != "0")) {
                        frei = false;
                    }
                    else {
                        if (unsafeWindow.global_x == "2") {
                            if (v % 12 == 0) {
                                ...
                                w = v + 1;
                                if (unsafeWindow.garten_kategorie[w] && (((unsafeWindow.garten_kategorie[w] != "v") && (unsafeWindow.garten_kategorie[w] != "hr") && (unsafeWindow.garten_kategorie[w] != "ex") && (unsafeWindow.garten_kategorie[w] != "alpin")) || unsafeWindow.garten_zeit[w] != "0")) {
                                    frei = false;
                                }
                                else {
                    ...
                    if (frei) {
                        ...
                        if (linecount > 11) {
                            v += 12 * (parseInt(unsafeWindow.global_y, 10) - 1);
                        }
        ...
        GM_logError("autoFarmPlant", "runId=" + runId + " v=" + v + " didPlant=" + didPlant + " isBot=" + isBot, "", err);
    }
}
function autoFarmPlantPremium(runId, step) {
    try {
                ...
                case 1: {
                    ...
                    for (var v = 1; v <= 120; v++) {
                        if ("v" == unsafeWindow.garten_kategorie[v] || "hr" == unsafeWindow.garten_kategorie[v] || "ex" == unsafeWindow.garten_kategorie[v] || "alpin" == unsafeWindow.garten_kategorie[v]) {
                            ...
                            if ((unsafeWindow.garten_kategorie[v] != "z") && (unsafeWindow.garten_kategorie[v] != "u")) {
                                leereFelder++;
                            }
                        }
                    }
        ...
        GM_logError("autoFarmPlantPremium", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoFarmWater(runId, v, didPlant, isBot) {
    try {
        if (bot.checkRun("autoFarmWater", runId, !isBot)) {
            if (isBot && !settings.get("account", "autoWater")) {
                autoZoneFinish(runId, $("gardencancel"));
            }
            else {
                ...
                if (unsafeWindow.premium == 1 || (unsafeWindow.currentuserlevel >= 4 && unsafeWindow.currentuserlevel < 10)) {
                    ...
                    if (v < 121) {
                        if ((unsafeWindow.garten_kategorie[v] == "v" || unsafeWindow.garten_kategorie[v] == "hr" || unsafeWindow.garten_kategorie[v] == "ex" || unsafeWindow.garten_kategorie[v] == "alpin") && (unsafeWindow.garten_zeit[v] != "0") && (isNaN(parseInt(unsafeWindow.garten_wasser[v], 10)) || (parseInt(unsafeWindow.garten_wasser[v], 10) + 86400 < unsafeWindow.Zeit.Server))) {
        ...
        GM_logError("autoFarmWater", "runId=" + runId + " v=" + v + " didPlant=" + didPlant + " isBot=" + isBot, "", err);
    }
}
...
function checkGuildJobProduct(zoneNrL) {
    try {
        ...
        GM_logError("productInGuildJop", "", "", err);
    }
}
    ...
    try {
        ...
        if (bot.checkRun("autoFarmStable", runId, !isBot)) {
            ...
            switch (step) {
                ...
                case 5:
                    {
                        if ((unsafeWindow.premium == 1) || (parseInt($("levelnum").innerHTML, 10) < 10)) {
        ...
        GM_logError("autoFarmStable", "runId=" + runId + " step=" + step + " didFeed=" + didFeed + " isBot=" + isBot + " sorte=" + sorte + " feedcounter=" + feedcounter + " maxFeed=" + maxFeed, "", err);
    }
}
function autoFarmFactory(runId, step) {
    try {
                ...
                case 3:
                    {
                        if ($("globalbox").style.display == "block") {
                            ...
                            if (checkGuildJobProduct(handled.zoneNrL)) {
                                ...
                                div = $("globalbox").querySelector('div[onclick*="createAdvancedProductionCommit(' + handled.zoneNr + ', ' + req[0][0] + ', ' + 'undefined' + ')"]');
                            }
                            action = function () {
                                //click($("globalbox").querySelector(".tt" + zoneList[handled.zoneNrL][0][0]));
                                click(div);
                            };
                            listeningEvent = "gameOpenGlobalBox";
                        }
        ...
        GM_logError("autoFarmFactory", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoFarmPony(runId, step) {
    try {
            ...
            switch (step) {
                case 1: {
                    if (unsafeData.zones.getBlock(handled.zoneNrS)) {
                        ...
                        if (unsafeData.zones.getEndtime(handled.zoneNrS) == NEVER) {
                        ...
                        else if (unsafeData.zones.getEndtime(handled.zoneNrS) <= unsafeWindow.Zeit.Server) {
                ...
                case 5: {
                    ...
                    else if ((unsafeData.readyZone[handled.zoneNrS][1] != "e") || !unsafeData.readyZone[handled.zoneNrS][2] || !unsafeData.pony_data) {
                        // Wait for response
                ...
                case 6: {
                    if (typeof unsafeWindow.pony_sel_farmi === 'undefined' || unsafeWindow.pony_sel_farmi == null) {
                        // Wait for response
                    ...
                    else if (unsafeWindow.pony_sel_farmi) {
        ...
        GM_logError("autoFarmPony", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoFarmFactoryOilKnittingTea(runId, step) {
    try {
        ...
        GM_logError("autoFarmFactoryOilKnittingTea", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoFarmFuelstation(runId, step) {
    try {
        ...
        GM_logError("autoFarmFuelstation_error", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoMegafield(runId, step) {
    try {
                ...
                case 1:
                    {
                        ...
                        if (!zoneWaiting[handled.zoneNrL] && // Befindet sich das Megafield nicht in der Liste der "behandle mich gerade nicht"-Zonen?
                            ...
                            ((("r" == help[1] || "e" == help[1]) && // siehe drüber => true
                                ...
                                ("e" == help[1] && PRODSTOP != zoneList[handled.zoneNrL][0][0]))) {
                ...
                case 2:
                    {
                        ...
                        if (!unsafeWindow.megafield_data.tour && // Die Tour wurde (bspw. in den Zeilen drüber) auf 0 gesetzt und ist damit beendet
                            ...
                            (PRODSTOP != zoneList[handled.zoneNrL][0][0] || !settings.get("account", "disableCropFields"))) {
                            // crop
                            autoMegafield(runId, step + 1);
                        }
                ...
                case 3:
                    {
                        ...
                        if (v_id == unsafeWindow.megafield_vehicle_id) {
                ...
                case 7:
                    {
                        ...
                        else if (unsafeWindow.megafield_plant_pid && zoneList[handled.zoneNrL][0][0] == unsafeWindow.megafield_plant_pid) {
                            var div;
                            if (unsafeWindow.premium == 1 && // Premium is active
                                settings.get("account", "botUseMegafieldPremiumPlanting") && // Option is activated
                                (div = $("megafield_products").querySelector('div[onclick*="dialogMegafield(\'autoplant\', 0, 0, 0, ' + zoneList[handled.zoneNrL][0][0] + ')"]'))) {
                                ...
                                var i = 0;
                                loop: for (var r = 1; r <= unsafeData.BUILDING_SIZE["megafield"][1]; r++) {
                ...
                case 8:
                    {
                        ...
                        else if (unsafeData.readyZone[handled.zoneNrS] && (!unsafeData.readyZone[handled.zoneNrS][2])) {
                        ...
                        if (unsafeWindow.megafield_data.tour) {
                            ...
                            window.setTimeout(function () {
                                for (var fz in zoneWaiting) {
                                    ...
                                    if (zoneWaiting[fz] <= now) {
                                        delete zoneWaiting[fz];
                                    } // Bereinige das zoneWaiting-Objekt um alle abgelaufenen Einträge
                                }
                                checkReadyZone();
        ...
        GM_logError("autoMegafield", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoWindmill(runId, step) {
    try {
            ...
            switch (step) {
                case 1:// init and open city
                    ...
                    if (handled.zoneNrL) {
                        ...
                        if (zoneList[handled.zoneNrL].length == 0) {
                            zoneList[handled.zoneNrL] = DEFAULT_ZONELIST_MILL_ARRAY;
                        }
                ...
                case 7:// wait for response
                    if (unsafeData.readyZone[handled.zoneNrS]) {
                        ...
                        if (isNaN(zoneList[handled.zoneNrL][0][1])) {
                            zoneList[handled.zoneNrL][0][1] = 1;
                        }
                        ...
                        if (zoneList[handled.zoneNrL][0][1] <= 0 && zoneList[handled.zoneNrL].length <= 1) {
                            zoneList[handled.zoneNrL] = DEFAULT_ZONELIST_MILL_ARRAY.clone();
                        }
                        // } else {
        ...
        GM_logError("autoWindmill", "runId=" + runId + " step=" + step, "", err);
    }
}
function checkPowerUp(mode) {
    try {
        ...
        if (settings.get("account", "powerUpActivate")) {
            for (var v in unsafeWindow.poweruprack) {
                if (!unsafeWindow.poweruprack.hasOwnProperty(v)) {
                    continue;
                }
...
function autoActivatePowerUp(runId, step, foundPowerUp) {
    try {
        ...
        GM_logError("autoActivatePowerUp", "runId=" + runId + " step=" + step, "", err);
    }
}
        ...
        if (!settings.get("account", "botUseForestry")) {
            ...
            if (zoneNrS == null) {
                ...
                if (unsafeData.readyZone[handled.zoneNrS][2]) {
                    switch (handled.zoneBuildingTyp) {
                        case "forest":
                            autoForestryForest(runId, 1, 1, false, true);
                            break;
                        case "sawmill":
                        case "carpentry":
                            autoForestryBuilding(runId, 1);
                            break;
                    }
                }
                else {
    ...
    try {
        ...
        if (settings.get("account", "botUseForestry") && bot.checkRun("autoForestryForest", runId, !isBot)) {
            ...
            switch (step) {
                case 1: {
                    if (zoneList[handled.zoneNrL].length == 0) {
                        zoneList[handled.zoneNrL] = DEFAULT_ZONELIST_ITEM_ARRAY.clone();
                    }
                    if (unsafeData.prodTyp[1][zoneList[handled.zoneNrL][0][0]] != "f1") {
                        zoneList[handled.zoneNrL] = DEFAULT_ZONELIST_ITEM_ARRAY.clone();
                    }
                    // open forestry
                ...
                case 2: {
                    var time = NEVER;
                    var NowServer = unsafeWindow.Zeit.Server;
                    for (var v = 0; v < unsafeWindow.forestry_area_time.length; v++) {
                ...
                case 3: {
                    if ((!isBot) || (zoneList[handled.zoneNrL][0][0] != PRODSTOP) || (!settings.get("account", "disableCropFields"))) {
                        ...
                        if (!unsafeData.zones.getProduction(handled.zoneNrS)[3]) {
                            ...
                            for (var i in unsafeWindow.forestry_area) {
                                if (!unsafeWindow.forestry_area.hasOwnProperty(i)) {
                                    continue;
                                }
        ...
        GM_logError("autoForestryForest", "runId=" + runId + " step=" + step + " field=" + field + " didPlant=" + didPlant + " isBot=" + isBot, "", err);
    }
}
function autoForestryBuilding(runId, step) {
    try {
            ...
            switch (step) {
                case 1: {
                    ...
                    if (handled.zoneNrF == "sawmill") {
                        if (unsafeData.prodTyp[1][zoneList[handled.zoneNrL][0][0]] != "f3") {
                            zoneList[handled.zoneNrL] = DEFAULT_ZONELIST_ITEM_ARRAY.clone();
                        }
                    }
                    else {
                        if ((unsafeData.prodTyp[1][zoneList[handled.zoneNrL][0][0]] != "f4") && (unsafeData.prodTyp[1][zoneList[handled.zoneNrL][0][0]] != "f5")) {
                            zoneList[handled.zoneNrL] = DEFAULT_ZONELIST_ITEM_ARRAY.clone();
                        }
                    }
        ...
        GM_logError("autoForestryBuilding", "runId=" + runId + " step=" + step, "", err);
    }
}
//22102016
function autobuyPetsParts(runId, step) {
    try {
        ...
        GM_logError("autobuyPetsParts", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoVehicles(runId, step, vehicle, garage) {
    try {
        ...
        GM_logError("autoVehicles", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoClothingDonation(runId, step) {
    try {
        if (bot.checkRun("autoClothingDonation", runId)) {
            ...
            switch (step) {
                ...
                case 3: {
                    ...
                    if (settings.get("account", "botUseClothingDonation") && unsafeData.latestClothingDonationLog["gambleInfo"][0]["gain"] > 0) {
                        ...
                        for (var prod in unsafeData.latestClothingDonationLog["in"]) {
                            if (!unsafeData.latestClothingDonationLog["in"].hasOwnProperty(prod)) {
                                continue;
                            }
                ...
                case 4: {
                    ...
                    if ($("globalbox").style.display == "none") {
                        autoClothingDonation(runId, step + 1);
                    }
                    else {
                        listeningEvent = "gameClothingDonationResponse";
        ...
        GM_logError("autoClothingDonation", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoDonkey(runId, step) {
    try {
        ...
        GM_logError("autoDonkey", "runId=" + runId + " step=" + step, "", err);
    }
}
function autoLottery(runId, step) {
    try {
        ...
        GM_logError("autoLottery", "runId=" + runId + " step=" + step, "", err);
    }
}
//olympiaRun 29122016
function autoOlympiaRun(runId, step) {
    try {
        ...
        GM_logError("autoOlympiaRun", "runId=" + runId + " step=" + step, "", err);
    }
}
//Siebenschläferrun
function autoSiebenSchlaeferRun(runId, step) {
    try {
        ...
        GM_logError("autoSiebenSchlaeferRun", "runId=" + runId + " step=" + step, "", err);
    }
}
...
function autoIceDelivery(runId, step) {
    try {
        ...
        GM_logError("autoIceDelivery", "runId=" + runId + " step=" + step, "", err);
    }
}
/* quest-bot needs review
function checkQuest(){
            ...
            if (zoneNrS == null) {
                ...
                if (zoneNrS.lastIndexOf(".") == -1) {
                    for (var i in unsafeData.readyZone) {
                        if (!unsafeData.readyZone.hasOwnProperty(i)) {
                            continue;
                        }
...
function autoFarmersmarketBuilding(runId, step, field) {
    try {
                        ...
                        case 4: {
                            if ((help = $("nursery_production_box")) && (help.style.display == "block") && handled.zoneNrF == "farmersmarket-2") {
                                ...
                                if (help2) {
                                    ...
                                    if (directionDown && (helpDown.style.display == "none")) {
                                        directionDown = false; // Switch to Scroll-Up-Mode
                                    }
                                    else if (!directionDown && (helpUp.style.display == "none")) {
                                ...
                                if (help2 && (!help2.className.match("important"))) {
                                    ...
                                    // link not visible, we need to scroll
                                    if (directionDown && !helpDown) {
                                        directionDown = false; // Switch to Scroll-Up-Mode
                                    }
                                    else if (!directionDown && !helpUp) {
                                        directionDown = true; // Switch to Scroll-Down-Mode
                                    }
                                    action = function () {
                                        if (directionDown) {
        ...
        GM_logError("autoFarmersmarketBuilding", "runId=" + runId + " step=" + step, "", err);
    }
}
...
function autoFarmersmarketVetTreatment(runId, step) {
    try {
        ...
        GM_logError("autoFarmersmarketVetTreatment", "runId=" + runId + " step=" + step, "", err);
    }
}
...
function autoFarmersmarketAnimalBreeding(runId, step, step4_6loop) {
    try {
            ...
            switch (step) {
                ...
                case 4: {
                    ...
                    for (var i in unsafeWindow.pets.care_set) {
                        if (!unsafeWindow.pets.care_set.hasOwnProperty(i)) {
                            continue;
                        }
        ...
        GM_logError("autoFarmersmarketAnimalBreeding", "runId=" + runId + " step=" + step, "", err);
    }
}
...
function autoFarmersmarketSpeedEating(runId, step) {
    try {
            ...
            switch (step) {
                ...
                case 3: {
                    ...
                    for (var i = 1; i <= 4 && !help; i++) {
                        if (item.blocks[i].block) {
                            continue;
                        }
                        for (var j in item.blocks[i].pin) {
                            if (!item.blocks[i].pin.hasOwnProperty(j)) {
                                continue;
                            }
        ...
        GM_logError("autoFarmersmarketSpeedEating", "runId=" + runId + " step=" + step, "", err);
    }
}
...
function autoFarmersmarketButterfly(runId, step) {
    try {
        ...
        GM_logError("autoFarmersmarketButterfly", "runId=" + runId + " step=" + step, "", err);
    }
}
...
function autoFoodworldBuilding(runId, step) {
    try {
        ...
        GM_logError("autoFoodworldBuilding", "runId=" + runId + " step=" + step, "", err);
    }
}
function checkFarmi(mode) {
    try {
        ...
        for (var i in unsafeWindow.farmisinfo[0]) {
            if (!unsafeWindow.farmisinfo[0].hasOwnProperty(i)) {
                continue;
            }
        ...
        GM_logError("checkFarmi", "mode=" + mode, "", err);
    }
}
function autoFarmi() {
    try {
            ...
            if (unsafeWindow.farm != 1) {
                ...
                var doFarmiInterval = window.setInterval(function (farmiNr, farmiAmount) {
                    try {
                        ...
                        GM_logError("autoFarmi", "", "farmiNr=" + farmiNr + " farmiAmount=" + farmiAmount, "", err);
                    }
...
function checkFoodworldFarmi() {
    try {
        ...
        GM_logError("checkFoodworldFarmi", "", "", err);
    }
}
function doGameSessionEnds() {
    click($("divSessionEnd"));
}
function doGameOtherAccReady() {
...
function drawZoneListTable(currZoneType) {
    try {
        ...
        newtable.addEventListener("mouseover", function (event) {
            ...
            while ((node != this) && (node.nodeName != "TD")) {
                ...
                if (node.parentNode.cells.length > 1) {
                    if (0 < posX) {
                        for (var v = 0; v < this.rows.length; v++) {
                            if (this.rows[v].cells[posX]) {
                                this.rows[v].cells[posX].style.backgroundColor = "lightblue";
                            }
                        }
                    }
                    if (0 < posY) {
                        for (var v = 0; v < this.rows[posY].cells.length; v++) {
                            if (this.rows[posY].cells[v]) {
                                this.rows[posY].cells[v].style.backgroundColor = "lightblue";
                            }
                        }
                    }
                }
        ...
        for (var fz in unsafeData.ALL_SLOTS) {
            ...
            if (getZoneType(fz) != currZoneType) {
                continue;
            }
        ...
        for (var lz in zoneList) {
            ...
            if ((!extendedList && currZoneType != getZoneType(lz)) || (extendedList && currZoneType != 1))
                continue;
            newtr = createElement("tr", { "style": "line-height:18px;" }, newtable);
            ...
            newtd.addEventListener("click", function () {
                try {
                    ...
                    GM_logError("drawZoneListTable/table1/td.click", "", "zoneNrL=" + zoneNrL, err);
                }
            }, false);
            for (var i = 0; i < zones.length; i++) {
                var fz = zones[i];
...
unsafeWindow.buildInfoPanelAutomat = function (mode, mode2) {
    try {
        var div;
        if (div = $("divQueueBoxClose")) {
            click(div);
        }
        if (div = $("divChooseBoxClose")) {
            click(div);
        }
        div = null;
        switch (mode) {
            case "automatChangelog":
                buildInfoPanelChangelog();
                break;
            case "automatOverview":
                buildInfoPanelOverview(mode2);
                break;
        }
    }
    catch (err) {
        GM_logError("buildInfoPanelAutomat", "", "", err);
    }
};
function buildInfoPanelChangelog() {
    try {
        ...
        GM_logError("buildInfoPanelChangelog", "", "", err);
    }
}
function buildInfoPanelOverview(mode) {
    try {
        ...
        for (var v in modeDefault) {
            if (!modeDefault.hasOwnProperty(v)) {
                continue;
            }
            if (mode[v] === undefined) {
                mode[v] = (modeOld[v] === undefined ? modeDefault[v] : modeOld[v]);
            }
        }
        ...
        newdiv1.addEventListener("click", function (filter) {
            return function (event) {
                try {
                    ...
                    GM_logError("farmlinks2.click", "event.ctrlKey=" + event.ctrlKey, "filter=" + filter + " thisFilter=" + thisFilter + " newFilter=" + newFilter, err);
                }
            };
        }(mode["filterZone"]), false);
        for (var farmNr = 1; farmNr <= unsafeData.ALL_ZONES["farm"].length / 6; farmNr++) {
            if (unsafeWindow.farms_data.blocked[farmNr]) {
                continue;
            }
        ...
        newdiv1.addEventListener("click", function (filter) {
            return function (event) {
                try {
                    ...
                    GM_logError("fieldlinks.click", "event.ctrlKey=" + event.ctrlKey, "filter=" + filter + " thisFilter=" + thisFilter + " newFilter=" + newFilter, err);
                }
            };
        ...
        for (var i in unsafeData.ALL_SLOTS) {
            if (!unsafeData.ALL_SLOTS.hasOwnProperty(i)) {
                continue;
            }
            if (unsafeData.zones.getBlock(i)) {
                continue;
            }
            if (i == "farmersmarket-5.5" || i == "farmersmarket-5.6" || i == "farmersmarket-5.7") {
                continue;
            }
        ...
        GM_logError("buildInfoPanelOverview", "", "", err);
    }
}
unsafeWindow.buildInfoPanelAutomatMenu = function (mode) {
    try {
        ...
        switch (mode) {
            case "automatOptions":
                buildInfoPanelOptions();
                break;
            case "automatZonePairing":
                buildInfoPanelZonePairing();
                break;
            case "automatDebugInfo":
                buildInfoPanelDebugInfo();
                break;
        }
        newdiv = null;
        newdiv1 = null;
        ...
        GM_logError("buildInfoPanelAutomatMenu", "", "", err);
    }
};
function buildInfoPanelOptions() {
    try {
        ...
        inp.addEventListener("change", function () {
            ...
            for (var v = 0; v < cell.children.length; v++) {
                ...
                for (var w = 0; w < emergencyPlants.length; w++) {
                    if (emergencyPlants[w] == cell.children[v].value) {
                        found = true;
                    }
                }
        ...
        */
        ...
        GM_logError("buildInfoPanelOptions", "", "", err);
    }
}
function buildInfoPanelOptionsDisabling() {
    try {
        ...
        GM_logError("buildInfoPanelOptionsDisabling", "", "", err);
    }
}
    ...
    try {
        ...
        for (var i in unsafeData.ALL_SLOTS) {
            ...
            if (i == "farmersmarket-3.1" || i == "farmersmarket-3.2" || i == "farmersmarket-3.3" ||
                i == "farmersmarket-7.1" || i == "farmersmarket-7.2" || i == "farmersmarket-7.3" || i == "farmersmarket-7.4" || i == "farmersmarket-7.5" || i == "farmersmarket-7.6") {
                continue;
            }
            zoneType = getZoneType(i);
            if (!slotCount[zoneType]) {
                slotCount[zoneType] = 0;
            }
            slotCount[zoneType]++;
        }
        for (var i in slotCount) {
            if (!slotCount.hasOwnProperty(i)) {
                continue;
            }
        ...
        GM_logError("buildInfoPanelZonePairing", "", "", err);
    }
}
function buildInfoPanelDebugInfo() {
    try {
        ...
        */
        ...
        GM_logError("buildInfoPanelDebugInfo", "", "", err);
    }
}
// ***************************************************************************************************
function do_main() {
    try {
        ...
        if (CHANGELOG[CHANGELOG.length - 1][0] != GM_getValue("automatChangelogShownVersion", "")) {
            ...
            alert2(container.innerHTML, getText("ok"), getText("showChangelog"), function () {
        ...
        */
        ...
        if ((typeof zoneList != "object") || (zoneList instanceof Array)) {
            zoneList = {};
        }
        // check consistence
        var bChanged = false;
        for (var fz in zoneList) {
            if (!zoneList.hasOwnProperty(fz)) {
                continue;
            }
        ...
        if (bChanged) {
            GM_setValue(COUNTRY + "_" + SERVER + "_" + USERNAME + "_zoneList", implode(zoneList, "settings/zoneList"));
        }
        zoneSettings = explode(GM_getValue(COUNTRY + "_" + SERVER + "_" + USERNAME + "_zoneSettings"), "settings/zoneSettings", {});
        if ((typeof zoneSettings != "object") || (zoneSettings instanceof Array)) {
            zoneSettings = {};
        }
        zoneToList = explode(GM_getValue(COUNTRY + "_" + SERVER + "_" + USERNAME + "_zoneToList"), "settings/zoneToList", {});
        if ((typeof zoneToList != "object") || (zoneToList instanceof Array)) {
            zoneToList = {};
        }
        // check consistence
        var bChanged = false;
        ...
        if (bChanged) {
            GM_setValue(COUNTRY + "_" + SERVER + "_" + USERNAME + "_zoneToList", implode(zoneToList, "settings/zoneToList"));
        }
        // check completeness
        for (var fz in unsafeData.ALL_SLOTS) {
            if (!unsafeData.ALL_SLOTS.hasOwnProperty(fz)) {
        ...
        */
        if (unsafeWindow.premium == 1 || parseInt(top.document.getElementById("levelnum").innerHTML, 10) < 10) {
            settings.set("account", "autoFeed", false);
        }
        ...
        newimg.addEventListener("click", function (event) {
            if (this.id == event.target.id) {
                ...
                if ($("divChooseBox").style.display = "block" && $("divChooseBoxInner").getAttribute("zoneNrF") == $("divQueueBoxInner").getAttribute("zoneNrF"))
                    click($("divChooseBoxClose"));
        ...
        newdiv = null;
        newimg = null;
        GM_addStyle(".rotate90 {-webkit-transform: rotate(90deg);-moz-transform: rotate(90deg);filter: progid:DXImageTransform.Microsoft.BasicImage(rotation=1);}\n" +
        ...
        document.addEventListener("gameFieldOpened", function () {
            try {
                if (settings.get("account", "autoPlant") && (!$("autoplantbutton"))) {
                    if ((unsafeWindow.premium != 1) || (unsafeWindow.currentuserlevel < unsafeWindow.autoplantlevel)) {
                ...
                GM_logError("eventListener:gameFieldOpened ", "", "", err);
            }
        }, false);
        ...
        document.addEventListener("gameOpenMegafield", function () {
            try {
                ...
                GM_logError("eventListener:gameOpenMegafield ", "", "", err);
            }
        }, false);
        ...
        document.addEventListener("gameMegafieldDataDone", function () {
            try {
                ...
                for (var num in data["area"]) {
                    if (!data["area"].hasOwnProperty(num)) {
                        continue;
                    }
                    item = data["area"][num];
                    if (!planted[item.pid]) {
                ...
                for (var pId in sumInQueue) {
                    if (!sumInQueue.hasOwnProperty(pId)) {
                        continue;
                    }
                    if (!planted[pId]) {
                        planted[pId] = 0;
                ...
                GM_logError("eventListener:gameMegafieldDataDone ", "", "", err);
            }
        }, false);
        ...
        document.addEventListener("gameCity2", function () {
            try {
                ...
                GM_logError("eventListener:gameCity2 ", "", "", err);
            }
        }, false);
        ...
        document.addEventListener("gameOpenForestry", function () {
            try {
                ...
                GM_logError("eventListener:gameOpenForestry ", "", "", err);
            }
        }, false);
        ...
        document.addEventListener("gameSawmillOpened", function () {
            try {
                ...
                GM_logError("eventListener:gameSawmillOpened", "", "", err);
            }
        }, false);
        ...
        document.addEventListener("gameCarpentryOpened", function () {
            try {
                ...
                GM_logError("eventListener:gameCarpentryOpened", "", "", err);
            }
        }, false);
                ...
                for (var v = 1; v <= 4; v++) {
                    if (v < 4) {
                        if (!unsafeData.zones.getBlock("foodworld-" + v + ".1")) {
                            drawAutomatIcon("foodworld-" + v, "foodworld-" + v + ".1", $("food_pos" + v), "position:absolute;left:90px;top:100px;");
                        }
                    }
                    else {
                        if (!unsafeData.zones.getBlock("foodworld-" + v + ".1")) {
                            drawAutomatIcon("foodworld-" + v, "foodworld-" + v + ".1", $("food_pos" + v), "position:absolute;right:52px;bottom:27px;");
                        }
                    }
                }
            }
            catch (err) {
                GM_logError("eventListener:gameFoodworldOpened ", "", "", err);
            }
        }, false);
        for (var v = 1; v <= 4; v++) {
            ...
            document.addEventListener("gameFoodworldOpened" + v, function (id) {
                return function () {
                    try {
                        ...
                        GM_logError("eventListener:gameFoodworldOpened" + id + "", "", "", err);
                    }
                };
            }(v), false);
        }
        function showCropWeed() {
            try {
                ...
                var newspan = null;
                for (var v = 1; v <= 120; v++) {
                    if (!velds[unsafeWindow.garten_prod[v]])
                        velds[unsafeWindow.garten_prod[v]] = new Array();
                    velds[unsafeWindow.garten_prod[v]].push(v);
                }
                for (var product in CROPCOST) {
                    if (!CROPCOST.hasOwnProperty(product)) {
                        continue;
                    }
                        ...
                        if (!$("cropWeed" + product)) {
                            ...
                            newspan.addEventListener("click", function () {
                                try {
                                    ...
                                    GM_logError("showCropWeed Icon Click ", "", "", err);
                                }
                            }, false);
                            ...
                            document.addEventListener("gameFieldDemolish", function (felds, product) {
                                return function () {
                                    try {
                                        ...
                                        GM_logError("autoCropUndefined/gameFieldDemolish", "felds=" + felds + " product=" + product, "", err);
                                    }
                                };
                            }(felds, product), false);
        ...
        document.addEventListener("gameOpenStable", function () {
            try {
                if (settings.get("account", "autoFeed")) {
                ...
                GM_logError("eventListener:gameOpenStable ", "", "", err);
            }
        }, false);
        err_trace = "listener gameOpenFactoryOilKnittingTea";
        document.addEventListener("gameOpenFactoryOilKnittingTea", function () {
            try {
                ...
                GM_logError("eventListener:gameOpenFactoryOilKnittingTea ", "", "", err);
            }
        }, false);
        err_trace = "listener gameOpenPony";
        document.addEventListener("gameOpenPony", function () {
            drawPonyIcons();
        }, false);
        err_trace = "listener gamePonyCropped";
        document.addEventListener("gamePonyCropped", function () {
            drawPonyIcons();
        }, false);
        err_trace = "listener gamePonyFed";
        document.addEventListener("gamePonyFed", function () {
            drawPonyIcons();
        }, false);
        err_trace = "listener gamePonyFarmiSet";
        document.addEventListener("gamePonyFarmiSet", function () {
            drawPonyIcons();
        }, false);
        err_trace = "listener gamePonySpeedup";
        document.addEventListener("gamePonySpeedup", function () {
            drawPonyIcons();
        }, false);
        function drawPonyIcons() {
            try {
                ...
                GM_logError("eventListener:gameOpenPony ", "", "", err);
            }
        }
        ...
        function updateFarmIcons() {
            try {
                ...
                GM_logError("updateFarmIcons", "", "", err);
            }
        }
        ...
        if (DEVMODE) {
            ...
            for (i in MenuCommand) {
                if (!MenuCommand.hasOwnProperty(i)) {
                    continue;
                }
        ...
        document.addEventListener("gameReallocateBuilding", function () {
            ...
            try {
                ...
                GM_logError("gameReallocateBuilding ", "", "", err);
            }
        ...
        // get dealer items
        document.addEventListener("gameUpdateFormuladealerRack", function () {
            try {
                var help = new Object();
                ...
                GM_logError("eventListener:gameUpdateFormuladealerRack ", "", "", err);
            }
        }, false);
        document.addEventListener("gameOpenWindmill", function () {
            try {
                // var help=new Object();
                ...
                GM_logError("eventListener:gameOpenWindmill ", "", "", err);
            }
        }, false);
        // Farmersmarket
        ...
        document.addEventListener("gameFarmersmarketOpened", function () {
            try {
                ...
                for (var v = 1; v <= 6; v++) {
                    if (v == 3 || v == 6) {
                        continue;
                    } //Automat-Icon blockieren (Farmersmarket-3)
                ...
                GM_logError("eventListener:gameFarmersmarketOpened ", "", "", err);
            }
        }, false);
        for (var v = 1; v <= 6; v++) {
                ...
                return function () {
                    try {
                        ...
                        for (var slot = 1; slot <= unsafeData.BUILDING_SLOTS[getZoneType(zoneNrF)]; slot++) {
                            ...
                            switch (id) {
                                case 2:
                                    container = $("nursery_slot_item" + slot);
                                    break;
                                case 3:
                                    container = $("megafruit_needs").children[slot - 1];
                                    break;
                                case 4:
                                    if (slot <= 4) {
                                    ...
                                    break;
                                case 5:
                                    container = $("vet_production_slot" + slot);
                                    break;
                        ...
                        GM_logError("eventListener:gameFarmersmarketOpened" + id + "", "", "", err);
                    }
                };
            }(v), false);
        }
                ...
                return function () {
                    try {
                        ...
                        for (var i in item.vehicles[id]) {
                            if (!item.vehicles[id].hasOwnProperty(i)) {
                                continue;
                            }
                        ...
                        DeliveryQuantity.addEventListener("keyup", function () {
                            ...
                            if (isNaN(valDeliveryQuantity)) {
                                ...
                                if (valDeliveryQuantity < 1) {
                                    valDeliveryQuantity = 1;
                                } // only allow values > 0
        ...
        */
        ...
        GM_logError("do_main", "", "err_trace=" + err_trace, err);
    }
}
function do_hilfe() {
    GM_addStyle(".queueButtonAddAll {display:inline-block;width:22px;height:16px;border:1px solid #6C441E;border-radius:5px;margin:0px 1px 0px 1px;}\n" +
    ...
    newa.addEventListener("click", function () {
        try {
            ...
            while (help = getText("automat_help_" + (i++))) {
                ...
                if (help[0]) {
                    createElement("b", {}, div, help[0]);
                }
                if (help[1]) {
                    createElement("p", {}, div, help[1]);
                }
            }
            container = null;
            div = null;
...
function manageVariables() {
    ...
    // 3: options, settings
    var variableInfo = { "GLOBAL": { "automatChangedata": ["Data version", 1],
    ...
    function buildManageVariables() {
        ...
        newinput.addEventListener("click", function () {
            ...
            if (data) {
                try {
                    ...
                    alert("Sorry. Can't read the entered string.\n" + err);
                }
            }
        }, false);
            ...
            if (data) {
                try {
                    ...
                    for (var v = 0; v < data.length; v++) {
                        ...
                        if (impType == "n") {
                            data[v][1] = parseFloat(data[v][1], 10);
                        }
                        else if (impType == "b") {
                            data[v][1] = (data[v][1] == "true");
                        }
                    }
                    buildImportTable(data);
                }
            ...
            for (var v = 0; v < help.length; v++) {
                switch (filter) {
                    ...
                    case "UNKNOWN":
                        ...
                        break;
                    case "USELESS":
                        if ((!variableInfo[help[v][2]][help[v][1]]) || (variableInfo[help[v][2]][help[v][1]][1] != 5)) {
                            continue;
                        }
                        break;
                    default: if (help[v][0] != filter) {
                        continue;
                    }
                }
                ...
                if (typeof help[v][4] == "number") {
                    help[v][3] = "n";
                }
                else if (typeof help[v][4] == "boolean") {
                    help[v][3] = "b";
                }
                else {
        ...
        function buildImportTable(data) {
            ...
            for (var v = 0; v < data.length; v++) {
                ...
                if (typeof data[v][3] == "number") {
                    data[v][2] = "n";
                }
                else if (typeof data[v][3] == "boolean") {
                    data[v][2] = "b";
                }
                newtr = createElement("tr", {}, newtable);
                newtd = createElement("td", {}, newtr);
...
function do_relogin(time) {
    ...
    if (document.getElementsByTagName("div").length > 10) {
        ...
        if (time > 0) {
            if (!newdiv) {
                newdiv = createElement("div", { "id": "automatDoRelogin", "class": "alertbubble tbig link" }, document.body);
            }
...
function start_script() {
    try {
        ...
        try {
            ...
            if (changedata < 1) {
                GM_setValue("automatChangedata", 1);
            }
        }
        catch (err) {
            GM_logError("changedata global", "", "", err);
        }
        // **************************************************
        // ChangeData Country
        ...
        try {
            ...
            if (changedata < 1) {
                GM_setValue(COUNTRY + "_automatChangedata", 1);
            }
        }
        catch (err) {
            GM_logError("changedata country", "", "", err);
        }
        // **************************************************
        // Multilingual
        err_trace = "text";
        ...
        if (loc) {
            ...
            try {
                ...
                if (changedata < 1) {
                    GM_setValue(COUNTRY + "_" + SERVER + "_automatChangedata", 1);
                }
            }
            catch (err) {
                GM_logError("changedata server", "", "", err);
            }
            // **************************************************
            // ChangeData User
            err_trace = "ChangeData Server";
            try {
                ...
                if (changedata < 1) {
                    GM_setValue(COUNTRY + "_" + SERVER + "_" + USERNAME + "_automatChangedata", 1);
                }
            }
            catch (err) {
                GM_logError("changedata user", "", "", err);
            }
            // **************************************************
            switch (PAGE) {
                //case "dbfehler":
        ...
        GM_logError("start_script", "", "err_trace=" + err_trace, err);
    }
}
;
