    ...
    try{
        ...
        if(bot.checkRun("autoFarm",runId)){
            ...
            if(zoneNrS==null){
                ...
                for (var c in unsafeWindow.farms_data.farms) {
                    if(!unsafeWindow.farms_data.farms.hasOwnProperty(c)){ continue; }
...
function buildInfoPanelOverview(mode){
    try{
        ...
        for (var farmNr in unsafeWindow.farms_data.farms) {
            if(!unsafeWindow.farms_data.farms.hasOwnProperty(farmNr)){ continue; }
        ...
        inp.addEventListener("click",function(){
            ...
            for(var slot=1;slot<=6;slot++){
                if( (slot == 1)||(slot>1 && (unsafeWindow.butterfly.data.data.slots[slot]) ) ) {
                    ...
                    zoneList["farmersmarket-7."+slot][0][0]=PRODSTOP;
                }
            }
            GM_setValue2(COUNTRY+"_"+SERVER+"_"+USERNAME+"_zoneList",implode(zoneList,"gameInfoPanelOpen/options/zoneList"));
            buildInfoPanelOptionsDisabling();
