...
declare var gameLocation;

const VERSION = GM_info["script"]["version"];
const neededVersionBeraterForRack = "2.5.3";
const neededVersionFunctionFileForRack = "2.1.7";
...
function closeInfoPanel() {
    try {
        ...
        div = null; div2 = null;
    } catch (err) { GM_logError("closeInfoPanel", "no parameters", err, "Error, when calling closeInfoPanel"); }
}
...
function buildInfoPanel(mode, shownTypes?) {
    ...
    if (!(div = $("infoPanel"))) {
        ...
        let gardenArea = $("garten_komplett"); // Area between products & quick jump icons and post box & xp bar
        ...
        let closeIcon = createElement("img", { "id": "infoPanelClose", "class": "link", "src": GFX + "close.jpg", "style": "position:absolute;top:8px;right:8px;width:20px;height:20px;" }, div);
        ...
        switch (mode) {
            case "rackoverview": {
                ...
                newdiv.addEventListener("mouseover", function (event) {
                    let mouseOverText: string = event.target.getAttribute("mouseOverText");
                    if (mouseOverText) {
                ...
                for (var v = 0; v < unsafeData.prodNameSort[0].length; v++) {
                    /** a product id */
                    let w: number = unsafeData.prodNameSort[0][v];
                    ...
                    if (showProduct) {
                        ...
                        if (bestand.length > 1) {
                            createElement("td", { "style": "text-align:right;" + (FARMNR == farm ? "background-color:#CCCCFF;" : (zCount % 2 == 0 ? "background-color:#eeeeee;" : "")) }, newtr, Farm.calculateCellValue(w, sumMainFarm, sumOfFarm5, sumOfFarm6));
                        }
                    }
                }
...
function do_main() {
    ...
    if (explodedBestand instanceof Array) {
        for (var i = 0; i < explodedBestand.length; i++) {
            ...
            bestand.push(new Farm(f.name, f.money, f.points, f.premium, f.level, f.farmMain, f.farm5, f.farm6));
        }
    }
