...
// @include        /^http|s:\/\/(|www\.|s\d+\.)au\.myfreefarm\.com\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)veselaferma\.com\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.com\.br\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.de\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.cz\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.dk\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.gr\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)migranjalinda\.es\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)mabelleferme\.fr\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.com\.hr\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.ae\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.ir\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.it\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)enkicsitanyam\.hu\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.nl\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.co\.nz\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.no\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)wolnifarmerzy\.pl\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)pt\.myfreefarm\.com\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)fermavesela\.ro\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)mojaderewnja\.ru\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.se\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)th\.myfreefarm\.com\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)tr\.myfreefarm\.com\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.co\.uk\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.com\/.*$/
// @include        /^http|s:\/\/(|www\.|s\d+\.)myfreefarm\.com\.vn\/.*$/
...
const neededVersionBerater="2.10.46";
const neededVersionFunctionFile="2.1.11";
    ...
    if(bot.checkRun("autoFarmPlant",runId,!isBot)){
        ...
        if(v<121){
            if((unsafeData.prodTyp[0][unsafeWindow.selected]=="v" || unsafeData.prodTyp[0][unsafeWindow.selected]=="hr" || unsafeData.prodTyp[0][unsafeWindow.selected]=="ex"|| unsafeData.prodTyp[0][unsafeWindow.selected]=="alpin")&&unsafeData.prodStock[stockNr][unsafeWindow.selected]&&unsafeData.prodStock[stockNr][unsafeWindow.selected]>0){
                ...
                var frei=true;
                if(unsafeWindow.garten_kategorie[v] && (((unsafeWindow.garten_kategorie[v]!="v") && (unsafeWindow.garten_kategorie[v]!="hr") && (unsafeWindow.garten_kategorie[v]!="ex") && (unsafeWindow.garten_kategorie[v]!="alpin")) || unsafeWindow.garten_zeit[v]!="0")){ frei=false; }
                else{
                    if(unsafeWindow.global_x=="2"){
                        if(v%12==0){ frei=false; }
                        else{
                            w=v+1;
                            if(unsafeWindow.garten_kategorie[w] && (((unsafeWindow.garten_kategorie[w]!="v") && (unsafeWindow.garten_kategorie[w]!="hr") && (unsafeWindow.garten_kategorie[w]!="ex") && (unsafeWindow.garten_kategorie[w]!="alpin")) || unsafeWindow.garten_zeit[w]!="0")){ frei=false; }
                            else{
                        ...
                        if (unsafeData.zones.getBlock(handled.zoneNrS)) {
                            ...
                            if ($("production_slot_info2" + handled.farmNr + "_" + handled.zoneNr + "_1").innerHTML == "") {
                                ...
                                for(var i=0;i<req.length;i++){
                                    ...
                                    } else if(unsafeData.prodStock[0][req[i][0]]<req[i][1]){
                                        startProductionButton = false;
                                    }
                                }
    ...
    if(bot.checkRun("autoFarmFactoryOil",runId)){
        ...
        case 1:{ // init
            ...
            if(unsafeData.zones.getBlock(handled.zoneNrS)){
                ...
                if(unsafeData.zones.getEndtime(handled.zoneNrS)==NEVER){ // empty
                    ...
                    window.setTimeout(autoFarmFactoryOil,settings.getPause(),runId,5);
                }
            }
        break;}
        case 2:{ // crop
            ...
            if(div){
                listeningEvent="gameFactoryOilCropped";
                action=function(){
                    unsafeData.readyZone[handled.zoneNrS][2]=false;
                    click(div);
                    div=null;
        ...
        case 3:{ // open slot
            if(!unsafeData.readyZone[handled.zoneNrS]){
                ...
                GM_logWarning("autoFarmFactoryOil","runId="+runId+" step="+step,"","LOGIC ERROR: onclick="+$("production_slot"+handled.farmNr+"_"+handled.zoneNr+"_"+handled.slot).onclick);
                autoFarmFactoryOil(runId,5);
            }else{
                listeningEvent="gameOpenGlobalCommitBox";
                action=function(){
...
try{
    ...
    if(settings.get("account","botUseVetTreatment")&&bot.checkRun("autoFarmersmarketVetTreatment",runId)){
        ...
        case 5:{ // //das rechte zu behandelnde Tier auswählen
            help=unsafeData.readyZone[handled.zoneNrS];
            //if(zoneList[handled.zoneNrL][0][0]==PRODSTOP){
            /*if(!botUseVetTreatment){
                //todo Abfrage ob ein Behandlungsslot frei ist
...
function buildInfoPanelOptions(){
    try{
        ...
        // *********** OlympiaRun ***********************************
        newtr=createElement("tr",{"style":"background-color:#b69162;"},newtable);
        createElement("th",{"colspan":"3"},newtr,"Olympia Run");
